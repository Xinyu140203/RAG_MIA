...
def split_dataset1(dataset: Dataset, split_symbol: list):
    assert len(split_symbol) == len(dataset)

    data = dataset.data
    data_split = {symbol: [] for symbol in set(split_symbol)}
    indexes_split = {symbol: [] for symbol in set(split_symbol)}

    for i, (x, symbol) in enumerate(zip(data, split_symbol)):
        data_split[symbol].append(x)
        indexes_split[symbol].append(i)
    for symbol in data_split:
        data_split[symbol] = Dataset(config=dataset.config, data=data_split[symbol])

    return data_split, indexes_split
