...
class LLMLinguaRefiner(BaseRefiner):
    ...
    def format_reference(self, retrieval_result):
        retrieval_results = "\n".join(item["contents"] for item in retrieval_result)
...
class AbstractiveRecompRefiner(BaseRefiner):
    ...
    def batch_run1(self, dataset, batch_size=2):
        # only use text
        retrieval_results = dataset.retrieval_result
        retrieval_results = [
            [doc_item["contents"] for doc_item in item_result]
            for item_result in retrieval_results
        ]
