...
class KGTraceRefiner(BaseRefiner):
    ...
    def get_answer_token_indices(self, tokenizer, num_choices, token_ids):
        """
        Obtain the index of the token corresponding to the option and a flag indicating
        whether a valid choice was found.
    
        Args:
            tokenizer: The tokenizer used to encode the choices.
            num_choices: The number of available choices (e.g., 4 for options A-D).
            token_ids: A tensor of shape (batch_size, sequence_length) containing token IDs.
    
        Returns:
            answer_token_indices (torch.Tensor): Tensor of shape (batch_size,) containing the index of the
                token corresponding to the selected option. If no valid option is found, it is set to
                the index of the last token in the sequence.
            valid_choice_flags (torch.Tensor): Boolean tensor of shape (batch_size,) where True indicates
                that a valid option was found, and False otherwise.
        """
        # Initialize the token ID to choice mapping if it hasn't been done yet
        if self.token_id_to_choice_map is None:
            self.token_id_to_choice_map = {}
            # Generate choices like ['A', 'B', 'C', 'D']
            choices = [chr(ord("A") + i) for i in range(num_choices)]
            for choice in choices:
                # Encode the choice without adding special tokens
                encoded = tokenizer.encode(choice, add_special_tokens=False)
                if encoded:
                    self.token_id_to_choice_map[encoded[0]] = choice
                # Encode the choice with a leading space (e.g., " A")
                encoded_space = tokenizer.encode(f" {choice}", add_special_tokens=False)
                if encoded_space:
                    self.token_id_to_choice_map[encoded_space[-1]] = choice
    
        # Initialize answer_token_indices with the index of the last token
        answer_token_indices = torch.zeros((token_ids.shape[0],), dtype=torch.long).fill_(token_ids.shape[1] - 1)
        # Initialize valid_choice_flags with False
        valid_choice_flags = torch.zeros((token_ids.shape[0],), dtype=torch.bool)
    
        # Iterate over each sequence in the batch
        for i in range(token_ids.shape[0]):
            for j in range(token_ids.shape[1]):
                token_id = token_ids[i, j].item()
                if token_id in self.token_id_to_choice_map:
                    answer_token_indices[i] = j
                    valid_choice_flags[i] = True
                    break  # Stop after finding the first valid choice
    
        return answer_token_indices, valid_choice_flags
    ...
    def get_reasoning_chain(self, all_query, all_doc_triples, triple_to_doc_ids):
        ...
        for query, doc_triples, generating_chain_examplars, final_chain_examplars, triple_doc_id_list in tqdm(
            ...
            for j in range(self.max_chain_length):
                print(f"-----------------------------------------------------------Running iteration {j+1} of {self.max_chain_length}")
                ...
                print("生成选项")
                print(generate_output ["responses"])
                answer_token_indices,valid_choice_flags = self.get_answer_token_indices(
                ...
                for i in range(len(paths)):
                    if paths_finished[i]:
                        ...
                        continue
                    print(topk_choices_probs[i])
                    if torch.all(torch.isnan(topk_choices_probs[i])) or not valid_choice_flags[i]:
                        print(
